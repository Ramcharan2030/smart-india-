...
const NAV_ITEMS = [
  { label: "SMART INDIA REALTORS", href: "#hero" },
  ...
  return (
    ...
    >
      ...
      <div className="hidden md:flex items-center gap-1 px-2 py-2 rounded-full glass-warm shadow-[0_4px_32px_rgba(138,106,62,0.08)]">
        ...
        >
          <span className="text-[var(--champagne)] font-bold text-xs">SIR</span>
        </button>
        {NAV_ITEMS.map((item) => (
          <button
            key={item.href}
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
            ...
            className="md:hidden absolute top-16 right-0 w-56 py-3 px-2 rounded-2xl glass-warm shadow-[0_8px_48px_rgba(138,106,62,0.12)]"
          >
            {NAV_ITEMS.map((item) => (
              <button
                key={item.href}
